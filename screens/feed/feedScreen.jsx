...
export default function FeedScreen({navigation}) {
	...
	function FeedItem({data}) {
		const {item} = data;
		return (<Pressable style={styles.itemArea} onPress={() => navigation.navigate('feedDetail', {data: item})}>
			<ImageBackground source={item.imgURI} resizeMode="cover" style={{flex: 1}} /> 
		</Pressable>)
	}
	return (<View style={{flex: 1, backgroundColor: '#f2f2f2'}}>
		{feedList ? <FlatList style={{flex: 1}} data={feedList}
			...
			numColumns = {3}
			renderItem={one => <FeedItem data={one} />}
			/>
		: <></>}
	</View>);
}
